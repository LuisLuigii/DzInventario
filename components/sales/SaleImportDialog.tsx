...
export default function SaleImportDialog({ open, onOpenChange }: {
  open: boolean
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const mapped = records.map((r: any) => ({
        ...
        cliente_discord: String(r.cliente_discord || ''),
        precio_venta: Number(r.precio_venta) || 0,
        fecha_venta: r.fecha_venta || new Date().toISOString().split('T')[0],
        tipo_garantia: r.garantia || '1 mes',
        observaciones: String(r.observaciones || ''),
        estado_garantia: 'Activa',
      })).filter(r => r.id_cuenta)

      const supabase = createClient()
      const { error } = await supabase.from('sales').insert(mapped)
      if (error) throw error
      queryClient.invalidateQueries({ queryKey: ['sales'] })
      setDone(true)
      toast.success(`${mapped.length} ventas importadas`)
    } catch (err: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => { onOpenChange(v); if (!v) setDone(false) }}>
      <DialogContent className="max-w-md bg-card border-border">
        <DialogHeader><DialogTitle>Importar Ventas</DialogTitle></DialogHeader>
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">Sube un CSV/Excel con columnas: <span className="font-mono text-xs">id_cuenta, cliente_discord, precio_venta, fecha_venta, garantia, observaciones</span></p>
          {done ? (
